  ...
  : QWidget(parent), d(new RadialVolumeWidgetPrivate(this))
{
  ...
  //setFont(QFont("helvetica", 20, QFont::Bold));
  d->level = 0;
...
void RadialVolumeWidget::paintEvent(QPaintEvent *)
{
  ...
  /*
  p.setPen(Qt::white); 
  QString disp_text = tr("%1%").arg(values[d->level]);
  p.drawText(c.x()-r, c.y()-r, r*2, r*2, Qt::AlignHCenter|Qt::AlignTop|Qt::TextSingleLine, disp_text);
  */
...
void RadialVolumeWidget::keyPressEvent(QKeyEvent *ev)
{
  switch (ev->key())
  {
    case Qt::Key_Up:
