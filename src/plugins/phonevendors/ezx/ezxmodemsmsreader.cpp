...
/*!
    ...
    \mainclass
    \brief The EzxModemSMSReader class provides SMS reading facilities for AT-based modems.
    ...
    EzxModemSMSReader implements the QSMSReader telephony interface.  Client
...
public:
    ...
    EzxModemService *service;
    int totalStore;
    QList<EzxSMSTaggedMessage *> pending;
    QList<EzxSMSTaggedMessage *> pseudoInbox;
    uint pseudoIndex;
    int pendingPosn;
    uint unreadCount;
    bool fragmentsOnly;
...
*/
EzxModemSMSReader::EzxModemSMSReader( EzxModemService *service )
    : QModemSMSReader(  service )
{
    d = new EzxModemSMSReaderPrivate();
    d->service = service;
    connect( service, SIGNAL(resetModem()), this, SLOT(resetModem()) );
    service->connectToPost( "smsready", this, SLOT(smsReady()) );

    service->secondaryAtChat()->registerNotificationType
        ( "+CMTI:", this, SLOT(newMessageArrived()) );

    service->secondaryAtChat()->registerNotificationType
        ( "+CDSI:", this, SLOT(newMessageArrived()) );

    connect( service->smsAtChat(),
...
*/
EzxModemSMSReader::~EzxModemSMSReader()
{
    delete d;
}
...
/*!
    \reimp
*/
void EzxModemSMSReader::firstMessage()
{
    if ( d->haveCached ) {
        d->pendingPosn = 0;
        nextMessage();
...
/*!
    \reimp
*/
void EzxModemSMSReader::nextMessage()
{
    ...
    if ( d->pendingPosn < d->pending.count() ) {
        ...
        EzxSMSTaggedMessage *tmsg = d->pending.at( d->pendingPosn++ );
...
/*!
    \reimp
*/
void EzxModemSMSReader::deleteMessage( const QString& id )
{
    if ( d->unreadList.contains( id ) ) {
        d->unreadList.removeAll( id );
        updateUnreadList();
    }
    QList<EzxSMSTaggedMessage *>::Iterator it;
	...
	if (timeIndex != -1)
        ...
        if ( store != "@@" && !messageStore().isEmpty() )
            d->service->secondaryAtChat()->chat( "AT+CPMS=\"" + store + "\"" );
        numDeleted = 0;
        while ( posn < (int)trimId.length() ) {
            ...
            if ( store == "@@" ) {
                ...
                d->service->secondaryAtChat()->chat( "AT+CMGD=" + index );
                ++numDeleted;
            }
        }
...
/*!
    \reimp
*/
void EzxModemSMSReader::setUnreadCount( int value )
{
    d->unreadCount = value;
    updateUnreadCount();
}

void EzxModemSMSReader::updateUnreadCount()
{
...
void EzxModemSMSReader::updateUnreadList()
{
...
*/
QString EzxModemSMSReader::messageStore() const
{
    return "SM";        // No tr
}
...
/*!
    ...
    \sa messageStore()
*/
QString EzxModemSMSReader::messageListCommand() const
{
...
*/
void EzxModemSMSReader::simDownload( const QSMSMessage& message )
{
...
void EzxModemSMSReader::check(bool force)
{
...
void EzxModemSMSReader::resetModem()
{
    if ( !d->initializing ) {
        d->initializing = true;
...
void EzxModemSMSReader::smsReady()
{
    if ( d->initializing ) {
        ...
        d->service->secondaryAtChat()->chat( "AT+CRRM" );
        d->service->secondaryAtChat()->chat( "AT+EPMS?" );
        d->service->secondaryAtChat()->chat( "AT+EMGL=4" );
...
void EzxModemSMSReader::nmiStatusReceived( bool, const QAtResult& result )
{
    ...
    new QRetryAtChat( d->service->secondaryAtChat(), line, 5 );

    // We are now initialized.
    d->initializing = false;
}

void EzxModemSMSReader::newMessageArrived()
{
        ...
        ( const QString& type, const QByteArray& pdu )
{
    if ( type.startsWith( "+CMT:" ) ) {
        ...
        d->service->smsAtChat()->chat("AT+ENMA=0,0,1,0");
...
void EzxModemSMSReader::cpmsDone( bool, const QAtResult& result )
{
...
void EzxModemSMSReader::joinMessages()
{
    int posn;
...
bool EzxModemSMSReader::joinMessages
    ( QList<EzxSMSTaggedMessage *>& messages, QStringList& toBeDeleted )
{
    QString multiId;
    uint part, numParts;
    QString multiId2;
    ...
    int posn, posn2;
    QList<EzxSMSTaggedMessage *> newList;
    EzxSMSTaggedMessage *tmsg;
    QList<EzxSMSTaggedMessage> partList;
    bool leftOvers;
...
bool EzxModemSMSReader::dispatchDatagram( EzxSMSTaggedMessage *m )
{
